import type { SourceId, Track } from "./track.ts";
...
function cleanText(value: string): string {
  return stripControls(value).trim().slice(0, TEXT_MAX);
}
...
function sanitizeArtworkUrl(value: unknown): string | null {
  ...
  if (!artworkHostAllowed(url.hostname)) {
    return null;
  }
  return url.href;
}
...
function artworkHostAllowed(hostname: string): boolean {
  ...
  if (host === "i.scdn.co" || host === "mosaic.scdn.co") {
    return true;
  }
